...
BUILD_URL="https://api.cirrus-ci.com/v1/artifact/build/${CIRRUS_BUILD_ID}/${CIRRUS_TASK_NAME}/binary/build.tgz"
...
echo curl -X POST -H "Zeek-HMAC: ${HMAC_DIGEST}" -H "Zeek-HMAC-Timestamp: ${TIMESTAMP}" \"http://os-perf:8080/zeek?branch=${CIRRUS_BRANCH}\&build=${BUILD_URL}\&build_hash=${BUILD_HASH}\"
